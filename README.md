...
- **Free-standing annotations on the twin.** Photo markers required a photo behind them; annotations don't — click anywhere on the model (3D or 2D) to drop a labeled, trade/severity-tagged pin, editable and deletable, stored in its own IndexedDB store (schema v2 → v3, additive). Rendered as a diamond so it's visually distinct from a photo marker at a glance, not just by color. "Add annotation" also has an "+ Add photo" sibling — attach an image while placing, and it becomes a real `Photo` run through the exact same `/analyze` → `/aggregate` pipeline as any batch-uploaded photo (`lib/analyze.ts`, reused unmodified), so it shows up in the punch list and dashboard too, not just as a twin-only pin.
- **A 2D plan view for the twin**, toggleable with 3D. A pure-SVG top-down projection of the same footprint — no three.js, so it never touches the twin's lazy 3D chunk — sharing the *exact same world coordinates* as the 3D view: a marker placed in one view lands at the identical position in the other (verified, not assumed). A floor/level filter (Basement/Ground/Level 2-4, derived from the same `FLOOR_HEIGHT` constants the 3D massing model uses) narrows the plan to one story at a time, and also sets the placement height for anything dropped while a specific floor is selected.
...
- The Vision page's real-time detector is scoped to one class (person) on a bundled floor-level clip or an uploaded photo — a working demonstration of the real-time-detection *architecture and data pipeline*, not a general safety-monitoring product. It does not classify PPE (hardhat/vest) — COCO, its training set, has no such class, and every ready-made hardhat classifier found traces back to Ultralytics' AGPL-3.0 pipeline or a metered cloud API, both incompatible with this page's scope. It is also not run against the aerial hero footage as a live feature — but the page shows *why*, with real numbers: the same model run against that footage (see Vision page) produced a false "boat" read and low-confidence "person" reads, evidence rather than assumption that aerial imagery needs purpose-built retraining a general model doesn't have.
